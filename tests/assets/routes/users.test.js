...
describe('Test \'/api/users\' path', () => {
    ...
    describe('Test \'/authenticate\' path', ()=>{
        const token = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJkYXRhIjp7Il9pZCI6IjVhYzUwNzgxYjkwZTdlMjkwODY3ZTRlOSIsInVzZXJuYW1lIjoidGVzdCIsInBhc3N3b3JkIjoiJDJhJDEwJDdIRUUuZS43dUJkNUFuUmtRZzdqS2V5ZlJVVUY1VWlMTXM2LkpsbGF0RFhPQkx6TjJwL21lIiwiZW1haWwiOiJ0ZXN0IiwiX192IjowfSwiaWF0IjoxNTIyODYxOTc3fQ.5Mj6I_a_FUhlrFgUzMIxhJbmD5ynsWaOI3XpMZRnc0k'
        it('Should respond with a 400 status if no header provided', (done) => {
            request(app).get('/api/users/authenticate').then((res) => {
        ...
        it('Should respond with a 400 status if invalid header provided', (done) => {
            request(app).get('/api/users/authenticate').set('x-auth', 'invalid').then((res) => {
                expect(res.statusCode).toBe(400)
                done()
            })
        })
        ...
        it('Should respond with a 200 status if valid header provided', (done) => {
            request(app).get('/api/users/authenticate').set('x-auth', token).then((res) => {
    ...
    describe('Test \'/register\ path', ()=>{
        it('Should respond with a 400 status if no body provided', (done) => {
            request(app).post('/api/users/register').then((res) => {
                expect(res.statusCode).toBe(400)
                done()
            })
        })
        ...
        it('Should respond with a 200 status if body provided', (done) => {
            request(app).post('/api/users/register').send({"username": "test", "password": "test", "email":"test@test.com" }).then((res) => {
                expect(res.statusCode).toBe(200)
                done()
            })
        })
    ...
    describe('Test \'/login\' path', ()=>{
        it('Should respond with a 400 status if no body provided', (done) => {
            request(app).post('/api/users/login').then((res) => {
                expect(res.statusCode).toBe(400)
                done()
            })
        })
        ...
        it('Should respond with a 400 status if invalid username provided', (done) => {
            request(app).post('/api/users/login').send({"username": "wrong", "password": "test" }).then((res) => {
                expect(res.statusCode).toBe(400)
                done()
            })
        })
        ...
        it('Should respond with a 400 status if invalid password provided', (done) => {
            request(app).post('/api/users/register').send({"username": "test", "password": "test", "email":"test@test.com" })
            .then(() => {
                request(app).post('/api/users/login').send({"username": "test", "password": "wrong" }).then((res) => {
                    expect(res.statusCode).toBe(400)
                    done()
                })
            })
        ...
        it('Should respond with a 200 status if credentials provided', (done) => {
            request(app).post('/api/users/register').send({"username": "test", "password": "test", "email":"test@test.com" })
            .then(() => {
                request(app).post('/api/users/login').send({"username": "test", "password": "test" }).then((res) => {
                    expect(res.statusCode).toBe(200)
                    done()
                })
            })
