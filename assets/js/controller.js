angular
    ...
    return {
      ...
      link: function(scope, elem, attrs) {
        $timeout(function() {
          ...
          scope.img =
            'http://gravatar.com/avatar/' + md5(attrs.email) + '?s=' + size
  ...
  .service('User', function($http, $localStorage) {
    this.authenticate = () => {
      return $http.get('/api/users/authenticate').then(res => {
        return res.data
      })
    }
  ...
  .controller('AppCtrl', function($scope, $location, User) {
    User.authenticate().then(res => {
      if (res.success) $scope.currentUser = res.message
    })
    $scope.logout = () => {
      User.logout()
  ...
  .controller('RegisterCtrl', function($scope, User, $location) {
    $scope.register = (username, email, password) => {
      User.register(username, email, password).then(res => {
        if (res.success) {
